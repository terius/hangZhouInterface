...
namespace Common
{
    public class ExcelHelper
    {
        ...
        public static DataTable GetData(string filePath, int headRowIndex = 0, int startCellIndex = 0, int endCellIndex = 0)
        {
            IWorkbook wk = null;
            bool isHss = false;
            try
            ...
            try
            {
                ...
                //获取标题
                var row1 = sheet.GetRow(headRowIndex);//获取标头  
                int cellCount = row1.LastCellNum; //第一行的列数  
                string excelColName;
                for (int j = 0; j < cellCount; j++)
                {
                    if (j >= startCellIndex && (endCellIndex == 0 || j <= endCellIndex))
                    {
                ...
                while (rows.MoveNext())
                {
                    ...
                    {
                        if (i >= startCellIndex && (endCellIndex == 0 || i <= endCellIndex))
                        {
                            ICell cell = row.GetCell(i);

                            if (cell == null || cell.ToString().ToUpper() == "NULL")
                            {
                                dr[tableColIndex] = null;
                            }
                            else
                            {
                                if (cell.CellType == CellType.Numeric && DateUtil.IsCellDateFormatted(cell))
                                {
                                    dr[tableColIndex] = cell.DateCellValue;
                                }
                                else if (cell.CellType == CellType.Formula)
                                {
                                    dr[tableColIndex] = cell.NumericCellValue.ToString();
                                }
                                else
                                {
                                    dr[tableColIndex] = cell.ToString().Trim();
                                }
                            }
                            tableColIndex++;
                        }
